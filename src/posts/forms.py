...
class PostModelForm(forms.ModelForm):
    ...
    content = forms.CharField(label='', widget=forms.Textarea(attrs={'rows':3,'placeholder': gettext_lazy("post_creation_prompt"), 'style':'background-color : #F0F2F5;'}))
...
class CommentModelForm(forms.ModelForm):
    body = forms.CharField(label='', widget=forms.TextInput(attrs={'placeholder': gettext_lazy("comment_creation_prompt")}))
    class Meta:
        model = Comment
        fields = ('body',)
